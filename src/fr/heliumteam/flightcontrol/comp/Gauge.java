...
public class Gauge extends JComponent {
    ...
    private BufferedImage createInstrumentBackground()
    {
        ...
        // ******************* DIGITAL PANEL ************************************************
        final Point2D DIGITAL1_START = new Point2D.Double(0, getHeight() * 0.60f);
        final Point2D DIGITAL1_STOP = new Point2D.Double(0, getHeight() * 0.60f + getHeight() * 0.13f);
        ...
        final Color[] DIGITAL1_COLORS =
        {
            new Color(0x333333),
            new Color(0x999999)
        };

        final LinearGradientPaint DIGITAL1_PAINT = new LinearGradientPaint(DIGITAL1_START, DIGITAL1_STOP, DIGITAL1_FRACTIONS, DIGITAL1_COLORS);

        g2.setPaint(DIGITAL1_PAINT);
        g2.fill(new RoundRectangle2D.Double(getWidth() * 0.31f, getHeight() * 0.60f, getWidth() * 0.39f, getHeight() * 0.13f, 6, 6));

        final Point2D DIGITAL2_START = new Point2D.Double(0, getHeight() * 0.605185f);
        final Point2D DIGITAL2_STOP = new Point2D.Double(0, getHeight() * 0.605185f + getHeight() * 0.12f);
        ...
        final Color[] DIGITAL2_COLORS =
        {
            new Color(0x000000),
            new Color(0x333333)
        };

        final LinearGradientPaint DIGITAL2_PAINT = new LinearGradientPaint(DIGITAL2_START, DIGITAL2_STOP, DIGITAL2_FRACTIONS, DIGITAL2_COLORS);

        g2.setPaint(DIGITAL2_PAINT);
        g2.fill(new RoundRectangle2D.Double(getWidth() * 0.31481f, getHeight() * 0.605181f, getWidth() * 0.38f, getHeight() * 0.12f, 5, 5));
        ...
        // ******************* MIN-MAX BLOCKS ***********************************************
        final Point2D BLOCK1_CENTER = new Point2D.Double(getWidth() * 0.365f + 2.0, getHeight() * 0.74f + 2.0);
        final Point2D BLOCK2_CENTER = new Point2D.Double(getWidth() * 0.61f + 2.0, getHeight() * 0.74f + 2.0);
        ...
        final Color[] BLOCK_COLORS =
        {
            new Color(0xFFFFFF),
            new Color(0x333333)
        };
        final RadialGradientPaint BLOCK1_PAINT = new RadialGradientPaint(BLOCK1_CENTER, 3, BLOCK_FRACTIONS, BLOCK_COLORS);
        final RadialGradientPaint BLOCK2_PAINT = new RadialGradientPaint(BLOCK2_CENTER, 3, BLOCK_FRACTIONS, BLOCK_COLORS);
        g2.setPaint(BLOCK1_PAINT);
        g2.fill(new Ellipse2D.Double(getWidth() * 0.365f, getHeight() * 0.74f, 6, 6));
        g2.setPaint(BLOCK2_PAINT);
        g2.fill(new Ellipse2D.Double(getWidth() * 0.61f, getHeight() * 0.74f, 6, 6));
        ...
        final Rectangle2D TEXT_BOUNDARY = METRICS.getStringBounds(getUnitString(), g2);
...
//
//        final Point2D HIGHLIGHT_START = FRAME2_START;
...
//        g2.fill(highlightShape);
    ...
    private void createTickmarks(Graphics2D g2)
    {
        ...
        for (double alpha = 2 * Math.PI - freeArea; alpha >= freeArea - step; alpha -= step)
        {
            ...
            if (counter == 5)
            {
                ...
                g2.setStroke(THIN_STROKE);                
                innerPoint = new Point2D.Double(CENTER.getX() + (RADIUS - MED_LENGTH) * sinValue, CENTER.getY() + (RADIUS - MED_LENGTH) * cosValue);
                outerPoint = new Point2D.Double(CENTER.getX() + RADIUS * sinValue, CENTER.getY() + RADIUS * cosValue);
            }
    ...
    private BufferedImage getDigit(int digit) {
        GraphicsConfiguration gfxConf = GraphicsEnvironment.getLocalGraphicsEnvironment().getDefaultScreenDevice().getDefaultConfiguration();
        BufferedImage IMAGE = gfxConf.createCompatibleImage(13, 26, Transparency.TRANSLUCENT);
    ...
    private void setDigits(double value)
    {
        ...
        GraphicsConfiguration gfxConf = GraphicsEnvironment.getLocalGraphicsEnvironment().getDefaultScreenDevice().getDefaultConfiguration();
        BufferedImage image = gfxConf.createCompatibleImage((int) (getWidth() * 0.38f), (int) (getHeight() * 0.12f), Transparency.TRANSLUCENT);
